...
description: Universal polish for web apps and sites built with React, TypeScript, and CSS (Tailwind and Motion noted, never required). Takes a competent interface to a beloved one within its own visual style; never introduces a palette, a typeface, or a motion personality. This skill should be used whenever the user is building, reviewing, auditing, or refining a web UI and wants it to feel considered, cohesive, premium, and detailed. Covers motion (easing, springs, linear(), stagger, exits), gestures and scroll, colour (OKLCH, contrast, dark mode, theming), typography, layout and spacing (4px grid), hierarchy, surfaces (shadows, borders, radii, images), buttons and controls, forms and inputs, overlays (modal, sheet, drawer, popover, tooltip, toast), haptics and sound on the web, icons, copy and naming, empty/loading/error/success states, onboarding, pricing and paywalls, touch and mobile web, performance as a design property (INP, CLS), accessibility, and marketing pages. Triggers on polish, feels off, feels generic, premium, craft, cohesive, make it better, audit, review UI, easing, cubic-bezier, linear(), spring, Motion, Framer Motion, hover, active, focus ring, shadow, border, radius, modal, dialog, drawer, sheet, popover, tooltip, toast, form, input, validation, button, icon, Lucide, typography, text-wrap, tabular-nums, OKLCH, contrast, APCA, dark mode, theme, empty state, skeleton, optimistic UI, INP, CLS, layout shift, touch, iOS Safari, safe-area, dvh, scroll snap, reduced motion, a11y, ARIA, landing page, marketing page, pricing page, design system, design tokens.
---
